...
BUILD_TMP=$(mktemp -d "$ROOT/tests/.accel-serialization.XXXXXX")
RELOCATED=$(mktemp -d)
...
cp "$ROOT/tests/accel/gpu-loop-accumulator.noct" "$BUILD_TMP/model.noct"
(
	cd "$ROOT"
...
check_run() {
	...
	diff "$ROOT/tests/accel/gpu-loop-accumulator.noct.out" "$RELOCATED/out"
