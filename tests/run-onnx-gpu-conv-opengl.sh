...
TMP=$(mktemp -d)
APP_TMP=$(mktemp -d "$ROOT/tests/.onnx-stage-g1-opengl.XXXXXX")
trap 'rm -rf "$TMP" "$APP_TMP"' EXIT HUP INT TERM
APP_REL=${APP_TMP#"$ROOT/"}

echo 'ONNX Stage-G1 generated Conv OpenGL tests:'
[ -x "$ONNX_ORACLE_PYTHON" ] || {
	echo "Missing pinned ONNX oracle Python: $ONNX_ORACLE_PYTHON"
	exit 1
}
python3 "$ROOT/tests/onnx2noct/make-stage-g1-fixtures.py" "$TMP/fixtures"
...
check_renderer() {
	...
	if grep -E 'ACCEL: OpenGL device:.*(llvmpipe|softpipe|Software Rasterizer)' \
		...
		echo 'FAIL Stage-G1 Conv selected a software renderer'
		exit 1
	fi
	...
	if grep -F 'CPU fallback' "$TMP/accel.log" >/dev/null; then
		echo 'FAIL Stage-G1 generated Conv used CPU fallback'
		exit 1
	fi
}
...
for name in conv-reuse conv-stride-relu; do
	...
	if [ "$name" = conv-stride-relu ]; then expected_pipelines=2; fi
...
if "$NOCT" -j0 --path="$TMP/generated-conv-reuse/gpu" \
	"$ROOT/tests/onnx2noct/stage-f-run.noct" \
	"$TMP/fixtures/conv-reuse.weights" \
	"$TMP/fixtures/conv-reuse.input.f32le" "$TMP/disabled-output.f32le" \
	> "$TMP/disabled.log" 2>&1; then
	echo 'FAIL Stage-G1 Conv ran with the accelerator disabled'
	exit 1
fi
...
for suffix in corrupt wrong-name; do
	...
	if "$NOCT" --accel=opengl -j0 \
		--path="$TMP/generated-conv-reuse/gpu" \
		"$ROOT/tests/onnx2noct/stage-f-run.noct" \
		"$TMP/fixtures/conv-reuse.$suffix.weights" \
		"$TMP/fixtures/conv-reuse.input.f32le" \
		"$TMP/$suffix-output.f32le" > "$TMP/weight-error.log" 2>&1; then
		echo "FAIL Stage-G1 accepted $suffix weights"
		exit 1
	fi
...
echo 'All ONNX Stage-G1 generated Conv OpenGL tests passed.'
