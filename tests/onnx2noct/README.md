...
/tmp/noct-onnx-oracle/bin/python -m pip install \
  -r tests/onnx2noct/oracle/requirements.txt
...
`tests/dnn/models.lock` pins their immutable URL, byte size, and SHA-256.

```sh
python3 tests/onnx2noct/oracle/download_models.py \
  --repository . \
...
/tmp/noct-onnx-oracle/bin/python \
  tests/onnx2noct/oracle/generate_fixtures.py \
...
```sh
/tmp/noct-onnx-oracle/bin/python \
  tests/onnx2noct/oracle/generate_model_oracles.py \
  tests/onnx2noct/oracle-data \
  --model mnist-12="$HOME/.cache/noct-onnx-models/mnist-12.onnx" \
  --model project-cifar-opset12=tests/onnx2noct/fixtures/models/project-cifar-opset12.onnx \
...
/tmp/noct-onnx-oracle/bin/python \
  tests/onnx2noct/oracle/build_models_lock.py \
  --repository . \
  --external-cache "$HOME/.cache/noct-onnx-models" \
  --oracles tests/onnx2noct/oracle-data/model-oracles.lock \
  --output tests/dnn/models.lock
```
...
`tests/dnn/models.lock` and must not be normalized away silently.
...
```sh
NOCT=../build-stage-c/noct tests/run-onnx2noct.sh
...
`tests/run-onnx-package-opengl.sh` additionally checks byte-repeated package
...
NOCT=./build-stage-b-opengl/noct \
CONVERTER_NOCT=./build-stage-c/noct \
sh tests/run-onnx-gpu-opengl.sh
```
...
sh tests/run-onnx-gpu-conv-opengl.sh
```
...
```sh
EGL_PLATFORM=surfaceless NOCT=./build-stage-b-opengl/noct \
CONVERTER_NOCT=./build-stage-c/noct \
ONNX_ORACLE_PYTHON=/tmp/noct-onnx-oracle/bin/python \
...
CONVERTER_NOCT=./build-stage-c/noct \
ONNX_ORACLE_PYTHON=/tmp/noct-onnx-oracle/bin/python \
sh tests/run-onnx-cifar-opengl.sh

EGL_PLATFORM=surfaceless NOCT=./build-stage-b-opengl/noct \
CONVERTER_NOCT=./build-stage-c/noct \
ONNX_ORACLE_PYTHON=/tmp/noct-onnx-oracle/bin/python \
sh tests/run-onnx-squeezenet-opengl.sh

NOCT=./build-stage-c/noct \
ONNX_ORACLE_PYTHON=/tmp/noct-onnx-oracle/bin/python \
sh tests/run-onnx-tinyyolo-blocker.sh
```
...
`tests/run-accel-serialization-hardening.sh` relocates raw `.nb`/`.nap`
...
CONVERTER_NOCT=./build-stage-c/noct \
ONNX_ORACLE_PYTHON=/tmp/noct-onnx-oracle/bin/python \
sh tests/run-onnx-gpu-contraction-opengl.sh
```
...
CONVERTER_NOCT=./build-stage-c/noct \
ONNX_ORACLE_PYTHON=/tmp/noct-onnx-oracle/bin/python \
sh tests/run-onnx-gpu-pool-opengl.sh
```
