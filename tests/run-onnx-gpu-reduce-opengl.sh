...
TMP=$(mktemp -d)
APP_TMP=$(mktemp -d "$ROOT/tests/.onnx-stage-g5-opengl.XXXXXX")
trap 'rm -rf "$TMP" "$APP_TMP"' EXIT HUP INT TERM
APP_REL=${APP_TMP#"$ROOT/"}

echo 'ONNX Stage-G5 generated reduction OpenGL tests:'
[ -x "$ONNX_ORACLE_PYTHON" ] || {
	echo "Missing pinned ONNX oracle Python: $ONNX_ORACLE_PYTHON"
	exit 1
}
python3 "$ROOT/tests/onnx2noct/make-stage-g5-fixtures.py" "$TMP/fixtures"
...
check_renderer() {
	...
	if grep -E 'ACCEL: OpenGL device:.*(llvmpipe|softpipe|Software Rasterizer)' \
		...
		echo 'FAIL Stage-G5 reduction selected a software renderer'
		exit 1
	fi
	...
	if grep -F 'CPU fallback' "$TMP/accel.log" >/dev/null; then
		echo 'FAIL Stage-G5 generated reduction used CPU fallback'
		exit 1
	fi
}

for name in reduce-sum-mean reduce-extrema softmax-logsoftmax; do
	expected_pipelines=3
	if [ "$name" = reduce-extrema ]; then expected_pipelines=2; fi
...
if "$NOCT" -j0 --path="$TMP/generated-reduce-sum-mean/gpu" \
	"$ROOT/tests/onnx2noct/stage-f-run.noct" \
	"$TMP/generated-reduce-sum-mean/model.weights" \
	"$TMP/fixtures/reduce-sum-mean.input.f32le" "$TMP/disabled-output.f32le" \
	> "$TMP/disabled.log" 2>&1; then
	echo 'FAIL Stage-G5 reduction ran with the accelerator disabled'
	exit 1
fi
grep -F '__gpu func requires the OpenGL backend' "$TMP/disabled.log" >/dev/null
[ ! -e "$TMP/disabled-output.f32le" ]

echo 'All ONNX Stage-G5 generated reduction OpenGL tests passed.'
