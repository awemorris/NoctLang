...
NOCT=${NOCT:-../build-stage-c/noct}
ROOT=$(CDPATH= cd -- "$(dirname "$0")/.." && pwd)
NOCT=$(CDPATH= cd -- "$(dirname "$NOCT")" && pwd)/$(basename "$NOCT")
TMP=$(mktemp -d)
APP_TMP=$(mktemp -d "$ROOT/tests/.onnx-stage-d.XXXXXX")
trap 'rm -rf "$TMP" "$APP_TMP"' EXIT HUP INT TERM

echo 'ONNX Stage-D structural reader tests:'
python3 "$ROOT/tests/onnx2noct/make-reader-fixtures.py" "$TMP/generated"
python3 "$ROOT/tests/onnx2noct/make-reader-fixtures.py" "$TMP/generated-2"
diff -qr "$TMP/generated" "$TMP/generated-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-normalize-fixtures.py" "$TMP/normalized"
python3 "$ROOT/tests/onnx2noct/make-normalize-fixtures.py" "$TMP/normalized-2"
diff -qr "$TMP/normalized" "$TMP/normalized-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-stage-f-fixtures.py" "$TMP/stage-f"
python3 "$ROOT/tests/onnx2noct/make-stage-f-fixtures.py" "$TMP/stage-f-2"
diff -qr "$TMP/stage-f" "$TMP/stage-f-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-stage-g1-fixtures.py" "$TMP/stage-g1"
python3 "$ROOT/tests/onnx2noct/make-stage-g1-fixtures.py" "$TMP/stage-g1-2"
diff -qr "$TMP/stage-g1" "$TMP/stage-g1-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-stage-g2-fixtures.py" "$TMP/stage-g2"
python3 "$ROOT/tests/onnx2noct/make-stage-g2-fixtures.py" "$TMP/stage-g2-2"
diff -qr "$TMP/stage-g2" "$TMP/stage-g2-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-stage-g3-fixtures.py" "$TMP/stage-g3"
python3 "$ROOT/tests/onnx2noct/make-stage-g3-fixtures.py" "$TMP/stage-g3-2"
diff -qr "$TMP/stage-g3" "$TMP/stage-g3-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-stage-g4-fixtures.py" "$TMP/stage-g4"
python3 "$ROOT/tests/onnx2noct/make-stage-g4-fixtures.py" "$TMP/stage-g4-2"
diff -qr "$TMP/stage-g4" "$TMP/stage-g4-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-stage-g5-fixtures.py" "$TMP/stage-g5"
python3 "$ROOT/tests/onnx2noct/make-stage-g5-fixtures.py" "$TMP/stage-g5-2"
diff -qr "$TMP/stage-g5" "$TMP/stage-g5-2" >/dev/null
python3 "$ROOT/tests/onnx2noct/make-stage-g6-fixtures.py" "$TMP/stage-g6"
python3 "$ROOT/tests/onnx2noct/make-stage-g6-fixtures.py" "$TMP/stage-g6-2"
...
run_reader() {
	...
	"$NOCT" "$mode" --path="$ROOT/tools/onnx2noct" \
		"$ROOT/tests/onnx2noct/reader.noct" "$path"
}
...
FIX="$ROOT/tests/onnx2noct/fixtures/models"
...
"$NOCT" -j0 --path="$ROOT/tools/onnx2noct" \
	"$ROOT/tests/onnx2noct/float-bits.noct" \
...
"$NOCT" -j0 --path="$ROOT/tools/onnx2noct" \
	"$ROOT/tests/onnx2noct/protobuf-depth.noct" \
...
run_normalize() {
	...
	path=$2
	"$NOCT" "$mode" --path="$ROOT/tools/onnx2noct" \
		"$ROOT/tests/onnx2noct/normalize.noct" "$path"
}
...
check_normalized() {
	...
	for mode in -j0 -j; do
		...
		diff "$ROOT/tests/onnx2noct/golden/stage-e/$golden.norm" \
			"$TMP/normalized.out"
	done
}
...
check_stage_f_source() {
	...
	for label in interp jit; do
		...
		diff "$ROOT/tests/onnx2noct/golden/stage-f/$name.model.noct" \
...
check_stage_f_contract_error() {
	...
	if "$NOCT" -j0 --path="$TMP/stage-f-unary-interp/gpu" \
		"$ROOT/tests/onnx2noct/stage-f-contract.noct" "$case_name" \
...
check_stage_g1_source() {
	...
	for label in interp jit; do
		...
		diff "$ROOT/tests/onnx2noct/golden/stage-g1/$name.model.noct" \
			"$output/gpu/model.noct"
...
check_stage_g2_source() {
	...
	for label in interp jit; do
		...
		diff "$ROOT/tests/onnx2noct/golden/stage-g2/$name.model.noct" \
			"$output/gpu/model.noct"
		test "$(find "$output" -maxdepth 1 -type f | wc -l)" -eq 2
		test -f "$output/gpu/model.noct"
		test -f "$output/gpu/main.noct"
...
check_stage_g3_source() {
	...
	for label in interp jit; do
		...
		diff "$ROOT/tests/onnx2noct/golden/stage-g3/$name.model.noct" \
			"$output/gpu/model.noct"
		test "$(find "$output" -maxdepth 1 -type f | wc -l)" -eq 2
		test -f "$output/gpu/model.noct"
		test -f "$output/gpu/main.noct"
...
for name in concat-strided-alias concat-axis-last; do
	for label in interp jit; do
		...
		diff "$ROOT/tests/onnx2noct/golden/stage-g4/$name.model.noct" \
			"$output/gpu/model.noct"
	done
...
for spec in 'reduce-sum-mean 3 4' 'reduce-extrema 2 3' 'softmax-logsoftmax 3 4'; do
	...
	for label in interp jit; do
		...
		diff "$ROOT/tests/onnx2noct/golden/stage-g5/$name.model.noct" "$output/gpu/model.noct"
	done
...
for label in interp jit; do
	...
	diff "$ROOT/tests/onnx2noct/golden/stage-g6/batchnorm-reference.model.noct" \
		"$output/gpu/model.noct"
done
...
python3 "$ROOT/tests/onnx2noct/verify-package.py" \
...
(
	...
	"$NOCT" --compile --app --path=tools/onnx2noct \
		"$APP_REL/reader.nap" tests/onnx2noct/reader.noct
)
...
(
	cd "$ROOT"
	"$NOCT" --compile --app --path=tools/onnx2noct \
		"$APP_REL/normalize.nap" tests/onnx2noct/normalize.noct
)
"$NOCT" -j0 "$APP_TMP/normalize.nap" \
	"$FIX/identity-opset12.onnx" > "$TMP/app-normalized.out"
diff "$ROOT/tests/onnx2noct/golden/stage-e/identity-opset12.norm" \
...
for name in unary broadcast-alias transpose-copy; do
	(
		...
		"$NOCT" --compile --app \
			...
			tests/onnx2noct/stage-f-run.noct
	)
done
...
for name in concat-strided-alias concat-axis-last; do
	( cd "$ROOT"; "$NOCT" --compile --app --path="$TMP/stage-g4-$name-interp/gpu" \
		"$APP_REL/stage-g4-$name.nap" tests/onnx2noct/stage-f-run.noct )
done
...
for name in reduce-sum-mean reduce-extrema softmax-logsoftmax; do
	( cd "$ROOT"; "$NOCT" --compile --app --path="$TMP/stage-g5-$name-interp/gpu" \
		"$APP_REL/stage-g5-$name.nap" tests/onnx2noct/stage-f-run.noct )
done

( cd "$ROOT"; "$NOCT" --compile --app --path="$TMP/stage-g6-batchnorm-interp/gpu" \
	"$APP_REL/stage-g6-batchnorm.nap" tests/onnx2noct/stage-f-run.noct )
