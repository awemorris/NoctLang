...
project-cifar-opset12)
	STAGE_LABEL=I.2
	MODEL="$ROOT/tests/onnx2noct/fixtures/models/project-cifar-opset12.onnx"
	EXPECTED_KERNELS=11
	EXPECTED_WEIGHTS=10
	EXPECTED_ELEMENTS=10
	EXPECTED_SHAPE=1,10
...
INPUT="$ROOT/tests/onnx2noct/oracle-data/$MODEL_ID.input.f32le"
EXPECTED="$ROOT/tests/onnx2noct/oracle-data/$MODEL_ID.output.f32le"
TMP=$(mktemp -d)
APP_TMP=$(mktemp -d "$ROOT/tests/.onnx-mnist-opengl.XXXXXX")
...
python3 "$ROOT/tests/onnx2noct/verify-locked-model.py" \
	"$ROOT/tests/dnn/models.lock" "$MODEL_ID" "$MODEL" >/dev/null
...
python3 "$ROOT/tests/onnx2noct/verify-package.py" "$TMP/package-a" \
	"$MODEL" >/dev/null
...
for mode in -j0 -j; do
	...
	check_renderer
	python3 "$ROOT/tests/onnx2noct/compare-model.py" "$actual" "$EXPECTED" \
...
"$NOCT" --accel=opengl -j --path="$TMP/relocated/gpu" \
	"$ROOT/tests/onnx2noct/model-benchmark.noct" \
...
python3 "$ROOT/tests/onnx2noct/compare-model.py" \
