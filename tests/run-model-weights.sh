...
python3 "$ROOT/tests/model-weights/make-nwt1-fixtures.py" "$TMP/fixtures"
python3 "$ROOT/tests/model-weights/make-nwt1-fixtures.py" "$TMP/fixtures-2"
cmp "$TMP/fixtures/valid.nwt1" "$TMP/fixtures-2/valid.nwt1"
cmp "$TMP/fixtures/errors.tsv" "$TMP/fixtures-2/errors.tsv"

for mode in '-j0' '-j'; do
	"$NOCT" "$mode" "$ROOT/tests/model-weights/binary-ok.noct" \
		"$TMP/floats.f32le" "$TMP/exact.bin" > "$TMP/binary.out"
	diff "$ROOT/tests/model-weights/binary-ok.noct.out" "$TMP/binary.out"
	"$NOCT" "$mode" "$ROOT/tests/model-weights/nwt1-write-read.noct" \
		"$TMP/written.nwt1" > "$TMP/nwt1.out"
	diff "$ROOT/tests/model-weights/nwt1-write-read.noct.out" "$TMP/nwt1.out"
done

cp "$ROOT/tests/model-weights/nwt1-write-read.noct" "$TMP/roundtrip.noct"
"$NOCT" --compile "$TMP/roundtrip.noct"
"$NOCT" -j0 "$TMP/roundtrip.nb" "$TMP/from-nb.nwt1" \
	> "$TMP/roundtrip-nb.out"
diff "$ROOT/tests/model-weights/nwt1-write-read.noct.out" \
	"$TMP/roundtrip-nb.out"
(
	cd "$TMP"
...
"$NOCT" -j0 "$ROOT/tests/model-weights/weights-gc.noct" \
	"$TMP/fixtures/valid.nwt1" "$hash" > "$TMP/gc.out"
diff "$ROOT/tests/model-weights/weights-gc.noct.out" "$TMP/gc.out"

tab=$(printf '\t')
while IFS="$tab" read -r name expected message; do
	if "$NOCT" -j0 \
		"$ROOT/tests/model-weights/weights-open-error.noct" \
...
for spec in \
	...
	mode=${spec%%|*}
	message=${spec#*|}
	if "$NOCT" -j0 "$ROOT/tests/model-weights/weights-errors.noct" \
...
for spec in \
	...
	if "$NOCT" -j0 "$ROOT/tests/model-weights/binary-errors.noct" \
...
${CC:-cc} ${TEST_CFLAGS:-} -I"$ROOT/include" -Wall -Wextra -Werror \
	"$ROOT/tests/model-weights/weights-vm-test.c" \
