 ...
 *
 * Common subexpression elimination over the structured CFG, designed
 * in docs/design/05-cse.md (read the Decision Log there before
 * changing anything).  The pass is a forward symbolic-execution-style
 * value numbering:
 *
 ...
 *    must-analysis.
 *  - Loop-body entry bumps the epoch unconditionally (D-CSE6: keeps
 *    cross-thread eventual visibility; a cached shared read must not
 *    cross a back edge) and re-keys every local assigned in the body
 *    (the kill-summary approximation of the loop-head meet).
 *
 ...
 * error ordering and effect ordering are unchanged (D-CSE4).
 *
 ...
 * could compile at level 0 but fail at level 2 — a D-CSE12 violation.
	...
	struct cse_ctx *ctx)
{
	...
	ctx->capture_count = 0;
	for (i = 0; i < ctx->value_count; i++) {
		struct cse_value *v = &ctx->values[i];
