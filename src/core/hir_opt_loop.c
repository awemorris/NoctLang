...
static bool hir_collect_expr(struct hir_analyze_ctx *ctx,
			     const struct hir_expr *expr,
			     bool write);
static bool hir_collect_block(struct hir_analyze_ctx *ctx,
			      struct hir_block *block);
static bool hir_block_within(const struct hir_block *block,
			     const struct hir_block *loop);
static bool hir_block_seen(struct hir_analyze_ctx *ctx,
			   struct hir_block *block);
static struct hir_scalar_effect *
hir_scalar_get(struct hir_analyze_ctx *ctx, const char *symbol);
static bool hir_record_scalar(struct hir_analyze_ctx *ctx,
			      const char *symbol,
			      bool write);
static bool hir_record_access(struct hir_analyze_ctx *ctx,
			      const char *symbol,
			      const struct hir_expr *index_expr,
			      bool write);
static bool hir_record_call(struct hir_analyze_ctx *ctx,
			    const struct hir_expr *expr,
			    bool pure);
static bool hir_collect_top_loops(struct hir_block *func,
				  struct hir_block *block,
				  const struct hir_memory_catalog *catalog,
				  FILE *fp,
				  const char *prefix,
...
bool
hir_memory_catalog_build_accel(
	struct hir_block *func,
	struct hir_memory_catalog *catalog)
{
	struct hir_memory_object object;
	...
	while (local != NULL) {
		if (local->storage_class == HIR_LOCAL_STORAGE_LOGICAL_BUFFER &&
			...
			object.source_line = local->declaration_line;
			object.element_kind = local->declared_packed_type;
			object.element_width = hir_packed_width(
			...
			if (local->is_parameter) {
				...
				if (param >= 0) {
					object.alias_kind =
						func->val.func.param_restricted[param] ?
						HIR_ALIAS_CHECKED_NOALIAS :
						HIR_ALIAS_MAY_ALIAS;
					object.readable =
						(func->val.func.param_accel_effect[param] &
						 ACCEL_EFFECT_READ) != 0;
					object.writable =
						(func->val.func.param_accel_effect[param] &
	...
	struct hir_loop_summary **summary)
{
	...
	*summary = NULL;
	...
	result->gpu_lowering_status = HIR_GPU_LOWERING_UNCHECKED;
	result->gpu_lowering_reason = HIR_PAR_REASON_NONE;
	...
	const char *prefix)
{
	...
	if (prefix == NULL || strcmp(prefix, "accel-analysis") != 0)
	...
	bool accel_catalog)
{
	...
	if (func == NULL || func->type != HIR_BLOCK_FUNC || fp == NULL)
		return false;
	if (!(accel_catalog ? hir_memory_catalog_build_accel(func, &catalog) :
			       hir_memory_catalog_build_func(func, &catalog)))
		return false;
	visited_count = 0;
	loop_count = 0;
