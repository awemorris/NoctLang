...
{
	return type == HIR_EXPR_PLOAD8U || type == HIR_EXPR_PLOAD8S ||
	       type == HIR_EXPR_PLOAD16U || type == HIR_EXPR_PLOAD16S ||
	       type == HIR_EXPR_PLOAD32;
}

static bool
unroll_is_store(int type)
{
	return type == HIR_EXPR_PSTORE8 || type == HIR_EXPR_PSTORE16 ||
	       type == HIR_EXPR_PSTORE32;
}

static bool
unroll_is_counter_term(const struct hir_expr *e, const char *counter)
{
	while (e != NULL && e->type == HIR_EXPR_PAR)
		e = e->val.unary.expr;
	return e != NULL && e->type == HIR_EXPR_TERM &&
	       e->val.term.term != NULL &&
	       e->val.term.term->type == HIR_TERM_SYMBOL &&
	       strcmp(e->val.term.term->val.symbol, counter) == 0;
}

static bool
unroll_int_term(const struct hir_expr *e, int *value)
{
	while (e != NULL && e->type == HIR_EXPR_PAR)
		e = e->val.unary.expr;
	if (e == NULL || e->type != HIR_EXPR_TERM ||
	    e->val.term.term == NULL ||
	    e->val.term.term->type != HIR_TERM_INT)
		return false;
...
static bool
unroll_parse_index(struct unroll_ctx *ctx, const struct hir_expr *e,
		   int *offset)
{
	int value;
	...
	if (e->type == HIR_EXPR_PLUS &&
	    unroll_int_term(e->val.binary.expr[0], &value) &&
	    unroll_is_counter_term(e->val.binary.expr[1], ctx->counter)) {
		*offset = value;
		return true;
	}
	return false;
...
unroll_check_term(struct unroll_ctx *ctx, const struct hir_expr *e)
{
	...
	if (term->type == HIR_TERM_SYMBOL) {
		...
		if (strcmp(term->val.symbol, ctx->counter) == 0) {
			ctx->reason = "counter-use";
			return false;
		}
		return true;
	}
	return term->type == HIR_TERM_INT || term->type == HIR_TERM_LONG;
}

static bool
unroll_check_expr(struct unroll_ctx *ctx, const struct hir_expr *e)
{
	int offset;

	if (e == NULL) {
		ctx->reason = "body-shape";
		return false;
	}
	if (++ctx->nodes > UNROLL_MAX_NODES) {
		ctx->reason = "body-budget";
		return false;
	}
	if (unroll_is_load(e->type) || unroll_is_store(e->type)) {
		...
		if (base == NULL || base->type != HIR_EXPR_TERM ||
		    ...
		    !unroll_parse_index(ctx, e->val.binary.expr[1], &offset) ||
		    offset > INT_MAX - (UNROLL_FACTOR - 1)) {
			ctx->reason = "affine-offset";
			return false;
		}
		ctx->accesses++;
		return true;
	...
	switch (e->type) {
	case HIR_EXPR_TERM:
		return unroll_check_term(ctx, e);
	case HIR_EXPR_PAR:
	case HIR_EXPR_NEG:
	case HIR_EXPR_NOT:
		return unroll_check_expr(ctx, e->val.unary.expr);
	case HIR_EXPR_LT:
	case HIR_EXPR_LTE:
	case HIR_EXPR_GT:
	case HIR_EXPR_GTE:
	...
	case HIR_EXPR_MINUS:
	case HIR_EXPR_MUL:
		return unroll_check_expr(ctx, e->val.binary.expr[0]) &&
		       unroll_check_expr(ctx, e->val.binary.expr[1]);
	case HIR_EXPR_DIV:
	case HIR_EXPR_MOD:
...
unroll_check_body(struct unroll_ctx *ctx)
{
	...
	if (!ctx->loop->val.for_.is_ranged ||
	    ...
	    ctx->loop->val.for_.is_vector || body == NULL ||
	    body->type != HIR_BLOCK_BASIC || !body->stop) {
		ctx->reason = "body-shape";
		return false;
	}
	for (stmt = body->val.basic.stmt_list; stmt != NULL;
		...
		if (stmt->lhs == NULL || !unroll_is_store(stmt->lhs->type)) {
			ctx->reason = "scalar-carried";
			return false;
		}
		if (!unroll_check_expr(ctx, stmt->lhs) ||
		    !unroll_check_expr(ctx, stmt->rhs))
			return false;
	}
	if (ctx->accesses == 0) {
		ctx->reason = "body-shape";
	...
	/* The current scalar register cache does not keep four variable-divide
	 * lanes live cheaply.  Measurements on x86_64 and Apple M5 showed a
	 * substantial regression, so keep this profitability guard until the
	 * lane scheduler/allocator can expose division ILP. */
	if (ctx->expensive_division &&
	    getenv("NOCT_UNROLL_EXPENSIVE") == NULL) {
		ctx->reason = "cost-division";
		return false;
	}
...
unroll_clone_term(const struct hir_term *term)
{
...
static struct hir_expr *
unroll_mk_term_symbol(const char *symbol)
{
	struct hir_expr *expr;
	struct hir_term *term;
...
static struct hir_expr *
unroll_mk_binary(int type, struct hir_expr *left, struct hir_expr *right)
{
	struct hir_expr *expr;
...
unroll_mk_index(struct unroll_ctx *ctx, int offset)
{
	...
	if (counter == NULL || offset == 0)
		return counter;
	if (offset > 0)
		return unroll_mk_binary(HIR_EXPR_PLUS, counter,
					unroll_mk_term_int(offset));
	if (offset == INT_MIN)
		return NULL;
	return unroll_mk_binary(HIR_EXPR_MINUS, counter,
				unroll_mk_term_int(-offset));
}

static struct hir_expr *
unroll_clone_expr(struct unroll_ctx *ctx, const struct hir_expr *expr,
		  int lane)
{
	...
	memset(copy, 0, sizeof(*copy));
	copy->type = expr->type;
	if (unroll_is_load(expr->type) || unroll_is_store(expr->type)) {
		   ...
		   int lane)
{
	...
	for (; source != NULL; source = source->next) {
		...
		if (copy->lhs == NULL || copy->rhs == NULL)
			return NULL;
		if (tail == NULL)
			head = copy;
		else
...
static bool
unroll_append_stmts(struct hir_stmt **head, struct hir_stmt **tail,
...
static struct hir_expr *
unroll_clone_bound(const struct hir_expr *expr)
{
	struct hir_expr *copy;
...
static struct hir_expr *
unroll_mk_mid(const struct hir_expr *lo, const struct hir_expr *hi)
{
	return unroll_mk_binary(HIR_EXPR_MINUS,
		unroll_clone_bound(hi),
		unroll_mk_binary(HIR_EXPR_AND,
			unroll_mk_binary(HIR_EXPR_MINUS,
				unroll_clone_bound(hi),
				unroll_clone_bound(lo)),
			unroll_mk_term_int(UNROLL_FACTOR - 1)));
}
...
unroll_transform(struct unroll_ctx *ctx)
{
	...
	tail_stmts = unroll_clone_stmts(ctx,
		source_body->val.basic.stmt_list, 0);
	if (tail_stmts == NULL)
		return false;
	...
	if (bulk_body == NULL || tail_body == NULL)
		return false;
...
static void
unroll_collect(struct hir_block *head, struct hir_block **loops, int *count)
{
	struct hir_block *block;
	struct hir_block *chain;

	for (block = head; block != NULL;) {
		switch (block->type) {
		case HIR_BLOCK_IF:
			for (chain = block; chain != NULL;
			     chain = chain->val.if_.chain_next)
				if (chain->val.if_.inner != NULL)
					unroll_collect(chain->val.if_.inner,
						       loops, count);
			break;
		case HIR_BLOCK_FOR:
			...
			if (block->val.for_.inner != NULL)
				unroll_collect(block->val.for_.inner, loops, count);
			break;
		case HIR_BLOCK_WHILE:
			if (block->val.while_.inner != NULL)
				unroll_collect(block->val.while_.inner, loops, count);
			break;
		default:
			break;
		}
